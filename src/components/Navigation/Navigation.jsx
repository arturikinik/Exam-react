...
const Navigation = () => {
  ...
  const linksHandler = (path) => {
    if (path !== activeLink) {
      // направлять пользователя куда-то
      router.push(path);
      setActiveLink(path);
    }
  };
  ...
  return (
    <header className="flex items-center bg-white shadow-md w-full fixed h-16">
      <div className="container">
        <div className="flex justify-between items-center">
          ...
          <nav className="inline-flex items-center gap-5">
            {navItems.map((item) => (
              <a
                onClick={() => linksHandler(item.path)}
                className={`cursor-pointer  ${
                  item.path === router.pathname ? "text-sky-600" : ""
                }`}
                key={item.path}
              >
                {item.name}
